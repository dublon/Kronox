...
public class PublicScreenGUI extends JFrame {
	...
	public class LIBscrolling extends Thread {
		...
		public void run() {
			...
			while (true) {
				if (scroll) {
					/*
					 * if (YScroll!=0){ //Not Firsttime LIBpanel.scrollRectToVisible(new
					 * Rectangle(0, 0, 1080, 1700));
					 * 
					 * }
					 */
					YScroll = 0;
					...
					// int max =
					// (((Constants.bookingsList.size()*libHeight))-(1920-topOffset-bottomOffset));
					try {
						Thread.sleep(10000);
	...
	public void drawTopBar() {
		JLabel picWelcomeToAg = new JLabel("");
		picWelcomeToAg.setIcon(new ImageIcon(PublicScreenGUI.class.getResource("/assets/ag.png")));
		picWelcomeToAg.setBounds(22, 24, 467, 160);
		mainPanel.add(picWelcomeToAg);

		JLabel picSLULogo = new JLabel("");
		picSLULogo.setIcon(new ImageIcon(PublicScreenGUI.class.getResource("/assets/slu.png")));
		picSLULogo.setBounds(905, 25, 155, 139);
		mainPanel.add(picSLULogo);
		...
		timeLabel.setFont(new Font("Futura Std Book", Font.PLAIN, 30));
		...
		courseLabel.setFont(new Font("Futura Std Book", Font.PLAIN, 30));
		courseLabel.setBounds(158, 13, 112, 31);
		panelBlackBox.add(courseLabel);

		JLabel roomLabel = new JLabel("Rum");
		roomLabel.setForeground(Color.WHITE);
		roomLabel.setFont(new Font("Futura Std Book", Font.PLAIN, 30));
		...
		lblClocklabel.setFont(new Font("Sweden Sans", Font.PLAIN, 79));
		...
		lblDatelabel.setFont(new Font("Sweden Sans", Font.PLAIN, 30));
	...
	public void drawBottomBar() {
		...
		lblKronoxLogo.setBounds(675, 17, 450, 100);
		blackBarLower.add(lblKronoxLogo);

		JLabel lblGroupAvailable = new JLabel("Group study rooms available | Antal lediga platser");
		lblGroupAvailable.setFont(new Font("Futura Std Book", Font.PLAIN, 25));
		lblGroupAvailable.setForeground(Color.BLACK);
		lblGroupAvailable.setBounds(708, 6, 366, 33);
		blackBarLower.add(lblGroupAvailable);

		JLabel lblGroupAg = new JLabel("Alnarpsgården:");
		lblGroupAg.setForeground(Color.BLACK);
		lblGroupAg.setFont(new Font("Futura Std Book", Font.PLAIN, 25));
		lblGroupAg.setBounds(708, 51, 115, 33);
		blackBarLower.add(lblGroupAg);

		JLabel lblGroupAgricum = new JLabel("Agricum:");
		lblGroupAgricum.setForeground(Color.BLACK);
		lblGroupAgricum.setFont(new Font("Futura Std Book", Font.PLAIN, 25));
		lblGroupAgricum.setBounds(708, 84, 115, 33);
		blackBarLower.add(lblGroupAgricum);
	}
	...
	public void drawSchedule() {
		...
		for (int i = 0; i < Constants.bookingsList.size(); i++) {
			int j = (Constants.bookingsList.get(i).getStartTime().get(Calendar.DAY_OF_YEAR)
			...
			if (j != lastNumber) {
				...
				JLabel lblNewDay = new JLabel("Imorgon | Tomorrow");
				lblNewDay.setHorizontalAlignment(JLabel.LEFT);
				Calendar c = Calendar.getInstance();

				nbrExtraRows = nbrExtraRows + 1;
				if (nbrExtraRows > 1 || (Constants.bookingsList.get(i).getStartTime().get(Calendar.DAY_OF_YEAR)
				...
				lblNewDay.setFont(new Font("Futura Std Book", Font.BOLD, 30));
			...
			// "+Constants.utb_kursinstans_grupper.get(courseNames.get(0)));
			String startTime = fix(Constants.bookingsList.get(i).getStartTime().get(Calendar.HOUR_OF_DAY)) + ":"
					+ fix(Constants.bookingsList.get(i).getStartTime().get(Calendar.MINUTE));
			String endTime = fix(Constants.bookingsList.get(i).getEndTime().get(Calendar.HOUR_OF_DAY)) + ":"
					+ fix(Constants.bookingsList.get(i).getEndTime().get(Calendar.MINUTE));
			...
			if ((i & 1) == 0) {
				if (now.after(minutesBefore) && now.before(Constants.bookingsList.get(i).getStartTime())) { // Startar
																											// snart
			...
			lblTime.setFont(new Font("Futura Std Book", Font.BOLD, 30));
			...
			lblCourse.setFont(new Font("Futura Std Light", Font.PLAIN, 30));
			...
			lblRoom.setFont(new Font("Futura Std Book", Font.BOLD, 30));
...
//	 public void initFonts() { try { Font Nb = Font.createFont(Font.TRUETYPE_FONT, new
//	 File("/assets/JosefinSans-Bold.ttf")).deriveFont(79f); GraphicsEnvironment ge =
//	 GraphicsEnvironment.getLocalGraphicsEnvironment();
//	 ge.registerFont(Nb.createFont(Font.TRUETYPE_FONT, new
//	 File("/assets/JosefinSans-Bold.ttf"))); } catch (IOException|FontFormatException e)
//	 {
//	
//	 } }
